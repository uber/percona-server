...
static Sys_var_ulong Sys_net_buffer_shrink_interval(
       ...
       GLOBAL_VAR(opt_net_buffer_shrink_interval), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, LONG_TIMEOUT), DEFAULT(0),
       BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG);
...
static Sys_var_enum Sys_threadpool_high_prio_mode(
  ...
  "transaction or a connection holding a table, user, or a global read or "
  "backup lock and whether it has any high priority tickets (see "
